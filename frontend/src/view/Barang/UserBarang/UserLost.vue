    ...
    <div class="h-screen bg-gray-300">
        ...
        <div class="h-screen flex flex-col pt-20">
            <div class="flex flex-col gap-2">
                <motion.h3 :initial="{scale: 0}" :animate="{scale: 1, transition: {duration: 1}}" class="text-center text-blue-950/80 text-3xl font-extrabold">Your Item Lost</motion.h3>
                <motion.div :initial="{scale: 0}" :animate="{scale: 1, transition: {duration: 1}}" class="grid grid-cols-2 gap-5 pl-5 pr-5">
                    <router-link to="/user/lost" class="border-3 border-yellow-600/80 text-center bg-white py-1 rounded-full text-yellow-600/80 text-xl font-bold">Lost item</router-link>
                    <router-link to="/user/found" class="border-3 border-white text-center bg-yellow-600/80 py-1 rounded-full text-white text-xl font-bold">Found Item</router-link>
                </motion.div>
            </div>
            <div>
                <div class="grid grid-cols-1 pt-5 pl-5 pr-5 gap-5">
                    <div v-if="lost.length">
                        <motion.button :initial="{scale: 0}" :animate="{scale: 1, transition: {duration: 1}}" @click="handleNavigate(item.id)" v-for="item in lost" :value="item.id" :key="item.id" class="flex gap-5 bg-gray-200 w-full p-5 rounded-xl">
